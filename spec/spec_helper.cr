...
# Mock things
class MoonScript::Installer::Repository
...
macro subject(method)
  subject = ->(sample : String) {
    MoonScript::Parser.new(sample, "TestFile.moonscript").{{method}}
  }
end
...
macro expect_ok(sample)
  it "Parses: " + {{"#{sample}"}} do
    ...
    result.should be_a(MoonScript::Ast::Node)
  end
end
...
def notify_lsp(method, message)
  ...
  server =
    MoonScript::LS::Server.new(in_io, out_io)
