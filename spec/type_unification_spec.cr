...
alias Checkable = MoonScript::TypeChecker::Checkable
alias Variable = MoonScript::TypeChecker::Variable
alias Record = MoonScript::TypeChecker::Record
alias Type = MoonScript::TypeChecker::Type
...
macro expect_result(a, b, expected)
  it "{{a.id}} vs {{b.id}}" do
    ...
    if node1 && node2
      result = MoonScript::TypeChecker::Comparer.compare(node1, node2)
...
describe MoonScript::TypeChecker do
  ...
  describe "Record Vs Record" do
    it "returns null" do
      ...
      MoonScript::TypeChecker::Comparer.compare(recorda, recorda).should be_a(Record)
      MoonScript::TypeChecker::Comparer.compare(recordb, recorda).should eq(nil)
    end
  end

  describe "Record vs Type" do
    it "returns null for different name" do
      MoonScript::TypeChecker::Comparer.compare(Record.new("Blah"), Type.new("Blaha")).should eq(nil)
      MoonScript::TypeChecker::Comparer.compare(Type.new("Blah"), Record.new("Blaha")).should eq(nil)
    end

    it "returns null record for same name" do
      MoonScript::TypeChecker::Comparer.compare(Record.new("Blah"), Type.new("Blah")).should be_a(Record)
      MoonScript::TypeChecker::Comparer.compare(Type.new("Blah"), Record.new("Blah")).should be_a(Record)
    end
  end
end
